...
for i=1:30
    ...
    conf.dataDir = 'data/' ;
    if exist(conf.dataDir)
        delete(fullfile(conf.dataDir,'*.mat'))
        delete(fullfile(conf.dataDir,'*.ps'))
    end
    ...
    conf.phowOpts = {'Verbose', 2, 'Sizes', 7, 'Step', 4} ;
    conf.clobber = false ;
    ...
    idx = sub2ind([length(classes), length(classes)],imageClass(selTest), imageEstClass(selTest)) ;
    confus = zeros(length(classes)) ;
    confus = vl_binsum(confus, ones(size(idx)), idx) ;
    aca(end+1)=mean(diag(confus)/conf.numTest);
end

% Plots
figure(1); clf;
plot(1:30,aca)
title('PHOW performance vs Sizes')
xlabel('Sizes')
ylabel('Performance (ACA)')
[M,I]=max(aca);
fprintf('The optimum number of sizes is %f with an ACA of %f',I,M)
saveas(gcf,'ACAvSizes17imgs_10classes_600words_10c_4steps.png')
% figure(1) ; clf;
% subplot(1,2,1) ;
...
% save([conf.resultPath '.mat'], 'confus', 'conf', 'aca') ;
